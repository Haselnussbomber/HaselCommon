...
internal sealed class VTableHookGenerator : IIncrementalGenerator
{
    ...
    internal sealed record VTableHookInfoGeneric(MethodInfo MethodInfo, string structName, int vTableIndex)
    {
        ...
        {
            builder.AppendLine($"{MethodInfo.Name}Hook = HaselCommon.Service.GameInteropProvider.HookFromAddress<{MethodInfo.Name}Delegate>(*(nint*)({structName}.StaticAddressPointers.VTable + 8 * {vTableIndex}), {MethodInfo.Name});");
        }
    }
