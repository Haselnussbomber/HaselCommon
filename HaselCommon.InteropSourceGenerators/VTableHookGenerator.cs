...
using HaselCommon.InteropSourceGenerators.Models;
using LanguageExt;
...
{
    private const string VTableHookAttributeName = "HaselCommon.Attributes.VTableHookAttribute`1";
    ...
    internal sealed record VTableHookInfoGeneric(MethodInfo MethodInfo, string structName, int vTableIndex)
    {
        ...
        {
            builder.AppendLine($"{MethodInfo.Name}Hook = HaselCommon.Service.GameInteropProvider.HookFromFunctionPointerVariable<{MethodInfo.Name}Delegate>((nint)({structName}.StaticAddressPointers.VTable + 8 * {vTableIndex}), {MethodInfo.Name});");
        }
    }
