...
internal readonly ref struct SeStringContext
{
    ...
    /// </summary>
    internal CultureInfo CultureInfo => Localization.GetCultureInfoFromLangCode(Language.ToCode());
    ...
    {
        if (index >= 0 && LocalParameters.Length > index)
        {
            value = LocalParameters[index].UIntValue;
            return true;
        }
    ...
    internal bool TryGetLStr(int index, out ReadOnlySeString value)
    {
        if (index >= 0 && LocalParameters.Length > index)
        {
            value = LocalParameters[index].StringValue;
            return true;
        }
