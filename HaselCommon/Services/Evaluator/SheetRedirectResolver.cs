...
public partial class SheetRedirectResolver
{
    ...
    public SheetRedirectFlags Resolve(ref string sheetName, ref uint rowId, ref uint colIndex)
    {
        ...
        switch (sheetName)
        {
            ...
            case "ObjStr":
                {
                    ...
                    switch (index)
                    {
                        ...
                        case 2: // Treasure
                            if (_excelService.TryGetRow<LSheets.Treasure>(rowId, out var treasureRow) &&
                                treasureRow.Unknown0.IsEmpty)
                                rowId = 0; // defaulting to "Treasure Coffer"
                            break;

                        case 3: // Aetheryte
                            rowId = _excelService.TryGetRow<LSheets.Aetheryte>(rowId, out var aetheryteRow) && aetheryteRow.IsAetheryte
                                        ? 0u // "Aetheryte"
                                        : 1; // "Aethernet Shard"
                            break;
            ...
            case nameof(LSheets.Treasure)
                when _excelService.TryGetRow<LSheets.Treasure>(rowId, out var treasureRow) &&
                     treasureRow.Unknown0.IsEmpty:
                rowId = 0; // defaulting to "Treasure Coffer"
                break;
            ...
            case "WeatherPlaceName":
                {
                    ...
                    if (_excelService.TryFindRow<LSheets.WeatherReportReplace>(r => r.PlaceNameSub.RowId == placeNameSubId, out var row))
            ...
            case nameof(LSheets.InstanceContent) when colIndex == 3:
                {
                    ...
                    if (_excelService.TryGetRow<LSheets.InstanceContent>(rowId, out var row))
                        rowId = row.Order;
                    break;
                }
            ...
            case nameof(LSheets.PartyContent) when colIndex == 2:
                {
                    ...
                    if (_excelService.TryGetRow<LSheets.PartyContent>(rowId, out var row))
            ...
            case nameof(LSheets.PublicContent) when colIndex == 3:
                {
                    ...
                    if (_excelService.TryGetRow<LSheets.PublicContent>(rowId, out var row))
                        rowId = row.ContentFinderCondition.RowId;
                    break;
                }
            ...
            case nameof(LSheets.AkatsukiNote):
                {
                    ...
                    if (_excelService.TryGetSubrow<LSheets.AkatsukiNote>(rowId, 0, out var row))
                        rowId = (uint)row.Unknown2;
                    break;
                }
        }
