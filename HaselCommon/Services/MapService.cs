...
public partial class MapService
{
    ...
    public ReadOnlySpan<byte> GetHumanReadableCoords(Level level)
    {
        var coords = GetCoords(level);
        return _commonTranslations.FormatCoordsXY(coords.X, coords.Y);
    }

    public string GetHumanReadableCoordsString(Level level)
    {
        var coords = GetCoords(level);
        return _commonTranslations.FormatCoordsXYString(coords.X, coords.Y);
    }
