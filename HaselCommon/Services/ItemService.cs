...
public partial class ItemService
{
    ...
    public bool CanTryOn(ItemHandle item)
    {
        ...
        if (!itemRow.EquipRestriction.TryGetValue(out var equipRaceCategoryRow))
            return false;
    ...
    public bool CanEquip(ItemHandle item, byte race, byte sex, byte classJob, short level, byte grandCompany, byte pvpRank, out uint errorLogMessage)
    {
        ...
        if (!TryGetItem(item, out var itemRow) || !itemRow.EquipRestriction.TryGetValue(out var equipRaceCategoryRow))
        {
