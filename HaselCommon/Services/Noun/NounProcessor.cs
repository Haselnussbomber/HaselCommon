...
{
    private readonly ConcurrentDictionary<NounParams, ReadOnlySeString> _cache = [];
    ...
    public ReadOnlySeString ProcessNoun(NounParams nounParams)
    {
        ...
        if (_cache.TryGetValue(nounParams, out var value))
            return value;

        var output = nounParams.Language switch
        {
            ClientLanguage.Japanese => ResolveNounJa(nounParams),
            ClientLanguage.English => ResolveNounEn(nounParams),
            ClientLanguage.German => ResolveNounDe(nounParams),
            ClientLanguage.French => ResolveNounFr(nounParams),
            _ => default,
        };

        _cache.TryAdd(nounParams, output);
    ...
    {
        var sheet = dataManager.Excel.GetSheet<RawRow>(nounParams.Language.ToLumina(), nounParams.SheetName);
        if (!sheet.TryGetRow(nounParams.RowId, out var row))
        {
            logger.LogWarning("Sheet {SheetName} does not contain row #{RowId}", nounParams.SheetName, nounParams.RowId);
            return default;
        }

        var attributiveSheet = dataManager.Excel.GetSheet<RawRow>(nounParams.Language.ToLumina(), nameof(LSheets.Attributive));
