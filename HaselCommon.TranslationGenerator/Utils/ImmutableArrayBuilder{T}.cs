...
// https://github.com/aers/FFXIVClientStructs/blob/f62fb05/InteropGenerator/Helpers/ImmutableArrayBuilder%7BT%7D.cs
// which got it from  the ComputeSharp project
// https://raw.githubusercontent.com/Sergio0694/ComputeSharp/main/src/ComputeSharp.SourceGeneration/Helpers/ImmutableArrayBuilder%7BT%7D.cs
...
namespace HaselCommon.TranslationGenerator.Utils;
...
internal struct ImmutableArrayBuilder<T> : IDisposable
{
    ...
    private sealed class Writer : IList<T>, IReadOnlyList<T>
    {
        ...
        {
            if (index < 0 || index > Count) ImmutableArrayBuilder.ThrowArgumentOutOfRangeExceptionForIndex();
        ...
        /// <inheritdoc />
        int IList<T>.IndexOf(T item) => Array.IndexOf(_array, item, 0, Count);
        ...
        /// <inheritdoc />
        bool ICollection<T>.Contains(T item) => Array.IndexOf(_array, item, 0, Count) >= 0;
        ...
        public Span<T> Advance(int requestedSize)
        {
            ...
            var span = _array.AsSpan(Count, requestedSize);
        ...
        {
            if (requestedSize > _array.Length - Count) ResizeBuffer(requestedSize);
        }
