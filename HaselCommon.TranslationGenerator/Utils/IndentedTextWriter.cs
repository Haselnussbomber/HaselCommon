...
// https://github.com/aers/FFXIVClientStructs/blob/f62fb05/InteropGenerator/Helpers/IndentedTextWriter.cs
...
internal sealed class IndentedTextWriter : IDisposable
{
    ...
    public Span<char> Advance(int requestedSize)
    {
        ...
        if (_builder.Count == 0 || _builder.WrittenSpan[^2..].SequenceEqual(DefaultNewLine.AsSpan()))
    ...
    {
        if (condition) Write(content.AsSpan(), isMultiline);
    }
    ...
    {
        if (condition) Write(content, isMultiline);
    }
    ...
    {
        if (skipIfPresent && _builder.WrittenSpan is [.., '\r', '\n', '\r', '\n']) return;

        _builder.AddRange(DefaultNewLine.AsSpan());
    }
    ...
    {
        if (condition) WriteLine(skipIfPresent);
    }
    ...
    {
        if (condition) WriteLine(content.AsSpan(), isMultiline);
    }
    ...
    public readonly ref struct WriteInterpolatedStringHandler
    {
        ...
        {
            if (value is not null) _writer.Write(value.ToString());
        }
