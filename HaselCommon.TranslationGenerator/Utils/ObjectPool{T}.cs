...
// https://github.com/aers/FFXIVClientStructs/blob/f62fb05/InteropGenerator/Helpers/ObjectPool%7BT%7D.cs
// which got it from  the ComputeSharp project
// https://raw.githubusercontent.com/Sergio0694/ComputeSharp/main/src/ComputeSharp.SourceGeneration/Helpers/ObjectPool%7BT%7D.cs
    ...
    where T : class
{
    ...
    public T Allocate()
    {
        ...
        if (item is null || item != Interlocked.CompareExchange(ref _firstItem, null, item)) item = AllocateSlow();
    ...
    private T AllocateSlow()
    {
        foreach (ref var element in _items.AsSpan())
        {
            ...
            if (instance is not null) if (instance == Interlocked.CompareExchange(ref element.Value, null, instance)) return instance;
        }
