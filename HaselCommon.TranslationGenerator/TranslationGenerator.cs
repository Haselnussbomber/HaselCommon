...
public class TranslationGenerator : IIncrementalGenerator
{
    ...
    {
        writer.WriteLine($@"public bool TryGetTranslation(ReadOnlySpan<byte> key, out ReadOnlySpan<byte> text)");

        using (writer.WriteBlock())
        {
            foreach (var kv in translations)
            {
                writer.WriteLine($@"if (key.SequenceEqual(""{kv.Key}""u8))");
                using (writer.WriteBlock())
                {
        ...
        using (writer.WriteBlock())
        {
            ...
            using (writer.WriteBlock())
            {
                foreach (var kv in translations)
                {
                    ...
                    writer.WriteLine($@"text = {MakeSafeIdentifier(kv.Key)}String;");
                    writer.WriteLine("return true;");
    ...
    private static void GenerateStringProperties(in IndentedTextWriter writer, in Dictionary<string, Dictionary<string, string>> translations)
    {
        foreach (var kv in translations)
        {
            GenerateStringProperty(writer, kv, true);
            GenerateStringProperty(writer, kv, false);
        }
    }

    private static void GenerateStringProperty(in IndentedTextWriter writer, in KeyValuePair<string, Dictionary<string, string>> translation, bool isUtf8)
    {
        ...
        var type = isUtf8 ? "ReadOnlySpan<byte>" : "string";
        var suffix = isUtf8 ? string.Empty : "String";

        if (translation.Value.Count > 1)
        {
            writer.WriteLine($@"public {type} {propertyName}{suffix} => _languageProvider.Language switch");
            ...
            foreach (var entry in translation.Value)
            {
                ...
                var stringLiteral = GetStringLiteral(text, isUtf8);
            ...
            writer.WriteLine($@"_ => {defaultTextLiteral ?? (isUtf8 ? "\"\"u8" : "string.Empty")}");
        ...
        {
            writer.Write($@"public {type} {propertyName}{suffix} => ");
            ...
            foreach (var entry in translation.Value)
            {
                ...
                writer.WriteLine($@"{GetStringLiteral(entry.Value, isUtf8)};");
            }
        }
    }
    ...
    private static string GetStringLiteral(string input, bool isUtf8)
    {
        var sb = new StringBuilder();
        sb.Append('"');
